...
if (isset($_SESSION['uid'])) {
  $user_info = $_SESSION['Username'] ?? addslashes($_SESSION['Username']);
   ...
   // mysql_select_db($database_MySQL, $MySQL);
    ...
    $id_llogfilial = "";  $v_whereidfil = "";
    $v_wheresql    = "";  $v_whereidcli = "";
...
// ---------------------------------------------------------------------------------
  ...
  if (($row_chn_info)&&($row_chn_info['nr'] != null)) {  $id_chn = $row_chn_info['nr'];  }
  ...
  if (($row_hd_info)&&($row_hd_info['nr'] != null))   {  $id_hd = $row_hd_info['nr'];    }
  ...
  if (($row_opb_info)&&($row_opb_info['nr'] != null)) {  $id_opb = $row_opb_info['nr'];  }
...
<table border="0" cellpadding="0" width="100%" cellspacing="0">
  <tr valign="middle">
    <td colspan="9" ><lable>Filiali:&nbsp;<select name="id_llogfilial" id="id_llogfilial">
...
<?php
    ...
    $row_filiali_info = $filiali_info->fetch_assoc();
       ...
       while ($row_filiali_info) {
?>
           <option value="<?php echo $row_filiali_info['id']; ?>" <?php if ($row_filiali_info['id'] == $id_llogfilial) { echo "selected"; } ?>><?php echo $row_filiali_info['filiali']; ?></option>
<?php
         $row_filiali_info = $filiali_info->fetch_assoc();;
       }
    mysqli_free_result($filiali_info);
...
?>
  ...
  <tr>
    <td colspan="9"><DIV class="ctxheading7"><B>Balanca sipas monedhave</B></DIV></td>
  </tr>
...
<?php
  ...
  $v_hyre_lek = 0;  $v_hyrje_lek = 0;  $v_dalje_lek = 0;  $v_dale_lek = 0;
...
?>
  ...
  <?php
    ...
    while ($row_h_menu) {
        ...
        if ($rownum == 1) {  $v_bg = "DBDBDB";  $rownum = 0; }
        else              {  $v_bg = "E9DFFD";  $rownum ++;  }
